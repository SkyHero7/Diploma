...
class Agrigation:
    def __init__(self, request):
        ...
        self.data = client['DB_diplom'].get_collection(
            'Collection_diplom').find()
        self.dt_from = datetime.strptime(
            request['dt_from'].replace(
                'T', ' '), '%Y-%m-%d %H:%M:%S')
        self.dt_upto = datetime.strptime(
            request['dt_upto'].replace(
                'T', ' '), '%Y-%m-%d %H:%M:%S')
    ...
    def group(self):
        ...
        data = sorted(
            self.filter(),
            key=lambda x: x['dt'])  # сортировка данных по времени
        if self.group_type == 'month':
            return {month: list(group) for month, group in groupby(
                data, key=lambda x: f"{x['dt'].year}-{x['dt'].month}-1")}, "%Y-%m-01T00:00:00"

        elif self.group_type == 'day':
            return {day: list(group) for day, group in groupby(
                data, key=lambda x: f"{x['dt'].year}-{x['dt'].month}-{x['dt'].day}")}, "%Y-%m-%dT00:00:00"

        elif self.group_type == 'hour':
            return {hour: list(group) for hour, group in groupby(
                data, key=lambda x: f"{x['dt'].year}-{x['dt'].month}-{x['dt'].day}-{x['dt'].hour}")}, "%Y-%m-%dT%H:00:00"
    ...
    def omissions(self):
        ...
        while start_date <= self.dt_upto:  # Диапазон времени
            ...
            if self.group_type == 'hour':
                range_time.append(
                    f'{start_date.year}-{start_date.month}-{start_date.day}-{start_date.hour}')
        ...
        if range_time != list(groupData.keys()):
            for time in range_time:
                try:
                    # Пересоздаем список для сортировки его данных
                    sorted_data[time] = groupData[time]
                except Exception:  # Если объект не найден, добавляем значение 0
                    if self.group_type == 'day':
                        sorted_data[time] = [{'__id': 'Objectid(None)', 'value': 0, 'dt': datetime(
    ...
    def dataset(self):
        ...
        # Получаем группы по времени (ключи от словаря)
        for time in sortedData:
            value = 0
            for data in sortedData[time]:  # получаем данные в этой группе
                value += data['value']

            self.resultData["labels"].append(
                datetime.strftime(
                    sortedData[time][0]['dt'],
                    self.formatTime))
